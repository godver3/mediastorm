...
type ServiceConfig struct {
	Workers int // Number of parallel queue workers (default: 4)
}
...
type Service struct {
	...
	// Worker notification
	workNotify chan struct{} // Notifies workers when new items are added to queue
...
func NewService(config ServiceConfig, metadataService *metadata.MetadataService, database *database.DB, poolManager pool.Manager, configGetter config.ConfigGetter) (*Service, error) {
	// Set defaults
	if config.Workers == 0 {
		config.Workers = 4
	}
	...
	processor := NewProcessor(metadataService, poolManager, configGetter)
	...
	service := &Service{
		...
		ctx:             ctx,
		cancel:          cancel,
		workNotify:      make(chan struct{}, 10), // Larger buffer for multiple notifications
...
func (s *Service) Stop(ctx context.Context) error {
	...
	// Close and recreate notification channel for potential restart
	close(s.workNotify)
	s.workNotify = make(chan struct{}, 10)
...
func (s *Service) AddToQueue(filePath string, relativePath *string, category *string, priority *database.QueuePriority) (*database.ImportQueueItem, error) {
	...
	// Notify workers that new work is available (non-blocking)
	// Send multiple notifications to ensure all workers are notified
	for i := 0; i < s.config.Workers; i++ {
		select {
		case s.workNotify <- struct{}{}:
		default:
			// Channel buffer full, skip additional notifications
			goto done
		}
	}
done:
...
func (s *Service) workerLoop(workerID int) {
	...
	for {
		...
		case <-s.workNotify:
			// New work available, process immediately
			s.processQueueItems(workerID)
		case <-s.ctx.Done():
...
func (s *Service) processQueueItems(workerID int) {
	...
	if item.RelativePath != nil {
		resultingPath, processingErr = s.processor.ProcessNzbFile(s.ctx, item.NzbPath, *item.RelativePath)
	} else {
		resultingPath, processingErr = s.processor.ProcessNzbFile(s.ctx, item.NzbPath, "")
	}
...
// notifyRcloneVFS is a stub - NovaStream doesn't use rclone VFS
func (s *Service) notifyRcloneVFS(item *database.ImportQueueItem, log *slog.Logger) {
	// NovaStream uses WebDAV instead of rclone, so no VFS notification needed
}
...
func (s *Service) ProcessItemInBackground(ctx context.Context, itemID int64) {
	go func() {
		...
		// Check if the initial context is already cancelled before we start
		select {
		case <-ctx.Done():
			log.Info("Context cancelled before processing started", "error", ctx.Err())
			return
		default:
		}

		// Use the service context for background work, not the HTTP request context
		// The request context will be cancelled when the HTTP response is sent,
		// but we need to continue processing in the background
		bgCtx := s.ctx
		...
		// Attempt to claim the item for immediate processing with retry logic
		const maxRetries = 3
		const retryDelay = 50 * time.Millisecond

		var claimed bool
		for attempt := 0; attempt < maxRetries; attempt++ {
			// Check for cancellation before each retry (using background context)
			select {
			case <-bgCtx.Done():
				log.Info("Context cancelled during claim attempts", "error", bgCtx.Err())
				return
			default:
			}

			claimed, err = s.database.Repository.TryClaimQueueItem(itemID)
			if err != nil {
				log.Error("Failed to claim queue item for background processing", "error", err, "attempt", attempt+1)
				if attempt == maxRetries-1 {
					return
				}
			...
			// If not claimed, wait a bit and try again
			if attempt < maxRetries-1 {
				log.Debug("Queue item not available for claiming, retrying", "queue_id", itemID, "attempt", attempt+1)
				time.Sleep(retryDelay)
			}
		}

		if !claimed {
			log.Info("Queue item already being processed by another worker after retries", "queue_id", itemID)
			return
		}

		// Check for cancellation before starting processing (using background context)
		select {
		case <-bgCtx.Done():
			log.Info("Context cancelled before processing NZB", "error", bgCtx.Err())
			// Release the claim since we're not processing
			_ = s.database.Repository.UpdateQueueItemStatus(itemID, database.QueueStatusPending, nil)
			return
		default:
		}

		item.Status = database.QueueStatusProcessing

		// Process the NZB file with the background context
		var (
			processingErr error
			resultingPath string
		)
		if item.RelativePath != nil {
			resultingPath, processingErr = s.processor.ProcessNzbFile(bgCtx, item.NzbPath, *item.RelativePath)
		} else {
			resultingPath, processingErr = s.processor.ProcessNzbFile(bgCtx, item.NzbPath, "")
		}
...
// AddNZBToQueue adds an NZB file to the queue for processing
func (s *Service) AddNZBToQueue(ctx context.Context, fileName string, nzbBytes []byte) (*database.ImportQueueItem, error) {
	s.log.InfoContext(ctx, "Adding NZB to queue", "fileName", fileName, "size", len(nzbBytes))
	...
	nzbPath := filepath.Join(tempDir, fmt.Sprintf("%d_%s", timestamp, safeFileName))

	// Write NZB bytes to file
	if err := os.WriteFile(nzbPath, nzbBytes, 0644); err != nil {
		return nil, fmt.Errorf("write NZB file: %w", err)
	}

	// Parse NZB to get file size estimate
	var estimatedSize int64
	if parsed, err := nzbparser.Parse(bytes.NewReader(nzbBytes)); err == nil && len(parsed.Files) > 0 {
		// Use the largest file as size estimate
		for _, f := range parsed.Files {
			var fileSize int64
			for _, seg := range f.Segments {
				fileSize += int64(seg.Bytes)
			}
	...
	// Check if context was cancelled before queueing
	select {
	case <-ctx.Done():
		// Clean up temp file since we're not queueing
		os.Remove(nzbPath)
		return nil, ctx.Err()
	default:
	}
	...
	// Create queue item
	queueItem := &database.ImportQueueItem{
		NzbPath:    nzbPath,
	...
	// Add to queue
	if err := s.database.Repository.AddToQueue(queueItem); err != nil {
		// Clean up temp file on error
		os.Remove(nzbPath)
		return nil, fmt.Errorf("add to queue: %w", err)
	}

	s.log.InfoContext(ctx, "NZB added to queue",
		"id", queueItem.ID,
		"fileName", fileName,
...
func (s *Service) ProcessNZBImmediatelyWithSource(ctx context.Context, fileName string, nzbBytes []byte, source ResolvedNZBSource) (string, error) {
	...
	// Write NZB bytes to file
	if err := os.WriteFile(nzbPath, nzbBytes, 0644); err != nil {
		return "", fmt.Errorf("write NZB file: %w", err)
	}

	// Clean up temp file when done
	defer os.Remove(nzbPath)

	s.log.InfoContext(ctx, "Processing NZB file immediately", "nzbPath", nzbPath)

	// Process the NZB file directly
	resultingPath, err := s.processor.ProcessNzbFile(ctx, nzbPath, "")
	if err != nil {
