...
export default function PlayerScreen() {
  ...
  // Subtitle sync debug overlay - set to true to debug subtitle timing
  const subtitleDebugEnabled = false;
  ...
  const mediaInfo = useMemo(() => {
    ...
    return {
      ...
      seasonNumber,
      episodeNumber,
      seriesId: mediaType === 'episode' ? seriesIdentifier || titleId || imdbId || tvdbId : undefined,
      seriesName: mediaType === 'episode' ? cleanSeriesTitle : undefined,
  ...
  useEffect(() => {
    ...
    console.log('[player] keyframeDelta effect triggered', {
      hookKeyframeDelta: newKeyframeDelta,
      refKeyframeDelta: keyframeDeltaRef.current,
    ...
    (rawTime: number, shouldShowControls: boolean = true) => {
      void (async () => {
        ...
        console.log('[player] seek requested', {
          ...
          duration: durationRef.current,
          currentTime: currentTimeRef.current,
          currentKeyframeOffset: actualPlaybackOffsetRef.current,
        ...
        if (shouldUseMetadataSubtitleOptions) {
          ...
          if (preselectedSubtitleTrack !== undefined) {
            ...
            if (preferredSubtitleMode !== undefined) {
              ...
              const selectedAudioStream = Number.isFinite(selectedAudioTrackIndex)
                ? metadata.audioStreams?.find((s: { index: number; language?: string }) => s.index === selectedAudioTrackIndex)
                : undefined;
        ...
        <View style={styles.container} nativeID="player-fullscreen-root">
          <View ref={videoWrapperRef} style={styles.videoWrapper}>
            <VideoPlayer
              ...
              subtitleSize={(userSettings?.playback?.subtitleSize ?? settings?.playback?.subtitleSize ?? 1.0) * (isAndroidTV ? 0.75 : Platform.OS === 'android' ? 0.6 : 1.0)}
              mediaType={mediaType}
